...
the `id` variable is no longer unique, so combining `baseline_df` with
`amyloid_longer_df` seems like it doesn’t make any sense. So, I still
used the unpivoted version of the amyloid dataset in the following
steps.
...
write_csv(baseline_amyloid_df, "data/baseline_amyloid_combined.csv")
```
...
resulting dataset is exported to `baseline_amyloid_combined.csv` under
the `data` directory.
